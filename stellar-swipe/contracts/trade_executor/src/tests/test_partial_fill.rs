...
/// Mint enough tokens for one trade (amount + fee).
fn funded_user(env: &Env, token: &Address) -> Address {
    let user = Address::generate(env);
    StellarAssetClient::new(env, token)
        .mint(&user, &(AMOUNT + DEFAULT_ESTIMATED_COPY_TRADE_FEE));
    user
}
...
fn full_fill_is_noop() {
    ...
    // No partial fill record should be stored.
...
fn unknown_trade_id_returns_trade_not_found() {
    let (env, exec_id, admin, _) = setup();
    let user = Address::generate(&env);
    // Inject trade with id=1 but report for id=99
...
fn partial_fill_updates_trade_status() {
    ...
    // Read back the pending trade and verify status.
