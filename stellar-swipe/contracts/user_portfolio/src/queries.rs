...
pub fn get_trade_history_page(
    ...
    assert!(limit > 0 && limit <= MAX_TRADE_HISTORY_LIMIT, "invalid page size");
    ...
    while index > 0 && entries.len() < limit {
        ...
        if let Some(position) = env.storage().persistent()
            ...
            entries.push_back(TradeHistoryEntry { trade_id: id, position });
        }
    }
    TradeHistoryPage { entries, next_cursor: if index > 0 { Some(index) } else { None } }
}
