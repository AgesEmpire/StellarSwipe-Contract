...
mod tests {
    ...
    fn digest_binds_all_fields() {
        let base = ctx(b"provider-a", b"signal-1", b"caller-1", b"registry-1");
        let base_digest = base.digest();

        assert_ne!(base_digest, ctx(b"provider-b", b"signal-1", b"caller-1", b"registry-1").digest());
        assert_ne!(base_digest, ctx(b"provider-a", b"signal-2", b"caller-1", b"registry-1").digest());
        assert_ne!(base_digest, ctx(b"provider-a", b"signal-1", b"caller-2", b"registry-1").digest());
        assert_ne!(base_digest, ctx(b"provider-a", b"signal-1", b"caller-1", b"registry-2").digest());
    }
}
